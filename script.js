...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const agencySections = document.querySelectorAll('.agency-section');
  const updateAgencyVisibility = () => {
    agencySections.forEach(section => {
      const hasVisibleCard = Array.from(section.querySelectorAll('.product-card')).some(card => card.style.display !== 'none');
